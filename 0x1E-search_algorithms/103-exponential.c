...
/**
 * _searching_in_array - prints the current searching array items
 *
 * @array: Sorted array
 * @l: starting position
 * @r: Ending position
 * Return: Nothing
 */
void _searching_in_array(int *array, int l, int r)
{
	int i;
...
/**
 * _binary_search - Search for an element in sorted array
 * using binary_search
 *
...
int exponential_search(int *array, size_t size, int value)
{
	...
	pos = _binary_search(&array[i / 2], end_pos - i / 2 + 1, value);
	if (pos == -1)
		return (-1);
	return (pos + i / 2);
}
